function [yy,forward,fwdDiff]=hermiteInterpolationFwd(x,y,xx)
% Ensure column input
x=x(:);
y=y(:).*x;
xx=xx(:);
...
for j=1:n2
    ...
    % Calculate Forward
    if fwdTrue
        forward(j)=b(i)+2*c(i)*(xx(j)-x(i))+3*d(i)*(xx(j)-x(i))^2;
    end
    
    % Calculate differentiated forward curve
    if fwdDiffTrue
        fwdDiff(j)=2*c(i)+6*d(i)*(xx(j)-x(i));
    end
end
end