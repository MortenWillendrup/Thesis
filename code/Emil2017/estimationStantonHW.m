...
% Load prepaymetns
isin='DK0009753469';mkt.bonds.isins{1};
prepay=mkt.bonds.(isin).prepayments(1:end-2)./100;
...
pricesDates=mkt.bonds.(isin).datesPrices;
...
%         end
        ...
        while abs(l-100*(1+model.X))>0.01&&k<14
            ...
            a=hermiteInterpolation(r,Ml,cibor(i));
            if l>min(100,a)*(1+model.X)
                lb=mb;
            else
                ub=mb;
            end
...
ub=[100;100;3];
tic
theta=fmincon(@(theta)GMMform(theta,prepay,Xstar,LAMBDA,abovePar),theta,[],[],[],[],lb,ub)
toc
[val,expectedPrepay]=GMMform(theta,prepay,Xstar,LAMBDA,abovePar);
