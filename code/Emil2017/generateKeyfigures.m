...
for j=1:size(mkt.bonds.isins,1)
    try
        ...
        % Save prices
        mkt.bonds.(isin).modelPrices=pricesTheo;
        mkt.bonds.(isin).OAD=OAD;
        mkt.bonds.(isin).OAC=OAC;
        mkt.bonds.(isin).Carry=Carry;
        mkt.bonds.(isin).OAS=OAS;
    catch
        fprintf('Isin failed: %s\n',mkt.bonds.isins{j})
    end
end
...
b=bar(dates,prepay,...
    ...
    'facealpha',1);
hold on
plot(dates,modelPrepayments(Alpha,Beta,lambda,Xstar,n)*100,...
    'color',[0 0 0],...
    'linewidth',2)
